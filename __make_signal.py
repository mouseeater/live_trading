...
def print_data():
    ...
    while True:
        time.sleep(1)
        nav = realtime_data.get("nav") # .get()의 기본값 None을 활용
        ...
        print(f"  NAV: {nav if nav is not None else 'N/A'} (수신: {nav_time})")
        print(f"  현재가: {price if price is not None else 'N/A'} (수신: {price_time})")
        
        # --- 요청하신 조건 확인 로직 ---
        # nav와 price 값이 모두 숫자인 경우에만 계산을 수행합니다.
        if nav is not None and price is not None:
            diff = nav - price
            print(f" ⚠️  괴리(NAV-현재가): {diff:+.2f}원") # 차이 값도 함께 출력하여 확인 용이
